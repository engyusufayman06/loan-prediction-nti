...
<img src="https://img.shields.io/badge/Baseline%20Test%20Accuracy-92.87%25-22C55E">
...
- six-model benchmark comparison;
...
The original benchmark compared six classifiers on the held-out test set:
...
However, it does **not** dominate every metric. Logistic Regression and SVM achieved higher class-1 recall in the original benchmark. That trade-off is intentionally documented rather than hidden.

> **Metric note:** the 92.87% figure is the documented **original benchmark result**. The newer tuned configuration is evaluated independently at runtime, so no new accuracy number is claimed until it has actually been executed and measured.
...
### Original XGBoost confusion matrix
...
For class 1, the original benchmark reported:
...
- Decision Tree: **100% training accuracy** vs **89.28% test accuracy** — a clear overfitting signal.
- Original XGBoost: **97.68% training accuracy** vs **92.87% test accuracy** — a train/test gap exists, but the held-out result remained the strongest benchmark result.
