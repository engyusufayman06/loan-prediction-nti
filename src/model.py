...
# Final notebook benchmark leader and deployable repository model.
HGB_CONFIG = {"random_state": 42, "max_iter": 300}
...
def train_model(data_path: str | Path) -> tuple[LoanModelBundle, dict[str, Any]]:
    ...
    # HistGradientBoosting follows the notebook's non-scaled tree-model path.
    scaler = None
    ...
    metrics: dict[str, Any] = {
        ...
        "raw_rows": int(len(data)), "clean_rows": int(len(x)),
        ...
        "feature_count": int(len(x.columns)), "feature_columns": x.columns.tolist(),
        "engineered_features": ENGINEERED_FEATURES.copy(), "model_config": HGB_CONFIG.copy(),
    }

    # HGB has no native feature_importances_; match the notebook's permutation approach.
    subset_x, subset_y = x_test.iloc[:1000], y_test.iloc[:1000]
    perm = permutation_importance(model, subset_x, subset_y, n_repeats=5, random_state=42, n_jobs=-1)
    vals = np.abs(perm.importances_mean)
    if vals.sum() > 0:
        vals = (vals / vals.sum()) * 100
    metrics["feature_importance"] = {name: float(value) for name, value in zip(x.columns, vals)}

    bundle = LoanModelBundle(model, scaler, x.columns.tolist(), category_encoders)
